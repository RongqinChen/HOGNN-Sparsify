...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='ZINCFull')
    parser.add_argument('--cfg', dest='cfg_file', type=str,
                        default='configs/zincfull.grit.connpoly.yaml')
...
def main():
    ...
    # ======== load dataset ==============
    train_set = ZINC(cfg.dataset.path, False, "train",
                     cfg.dataset.full_pre_transform, cfg.dataset.inmemory_transform, cfg.dataset.onthefly_transform)
    valid_set = ZINC(cfg.dataset.path, False, "val",
                     cfg.dataset.full_pre_transform, cfg.dataset.inmemory_transform, cfg.dataset.onthefly_transform)
    test__set = ZINC(cfg.dataset.path, False, "test",
                     cfg.dataset.full_pre_transform, cfg.dataset.inmemory_transform, cfg.dataset.onthefly_transform)
